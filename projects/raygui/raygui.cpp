...
Pixel RAYWHITE = Pixel(0xffffffff);
...
void draw()
{
 ...
 //----------------------------------------------------------------------------------
    ...
    GuiStatusBar((GRectangle) { 0, GetScreenHeight() - 20, GetScreenWidth(), 20 }, "This is a status bar");
    ...
    if (showMessageBox)
    {
        ...
        int result = GuiMessageBox((GRectangle) { GetScreenWidth() / 2 - 125, GetScreenHeight() / 2 - 50, 250, 100 }, GuiIconText(RICON_EXIT, "Close Window"), "Do you really want to exit?", "Yes;No");
    ...
    {
        DrawRectangle(0, 0, width, height, Fade(RAYWHITE, 0.8f));
        int result = GuiTextInputBox((GRectangle) { GetScreenWidth() / 2 - 120, GetScreenHeight() / 2 - 60, 240, 140 }, GuiIconText(RICON_FILE_SAVE, "Save file as..."), "Introduce a save file name", "Ok;Cancel", textInput);
