...
typedef struct Font {
    ...
    GRectangle* recs;        // Characters rectangles in texture
...
RAYGUIDEF bool GuiWindowBox(GRectangle bounds, const char* title);                                       // Window Box control, shows a window that can be closed
RAYGUIDEF void GuiGroupBox(GRectangle bounds, const char* text);                                         // Group Box control with text name
RAYGUIDEF void GuiLine(GRectangle bounds, const char* text);                                             // Line separator control, could contain text
RAYGUIDEF void GuiPanel(GRectangle bounds);                                                              // Panel control, useful to group controls
RAYGUIDEF GRectangle GuiScrollPanel(GRectangle bounds, GRectangle content, Vector2* scroll);               // Scroll Panel control

// Basic controls set
RAYGUIDEF void GuiLabel(GRectangle bounds, const char* text);                                            // Label control, shows text
RAYGUIDEF bool GuiButton(GRectangle bounds, const char* text);                                           // Button control, returns true when clicked
RAYGUIDEF bool GuiLabelButton(GRectangle bounds, const char* text);                                      // Label button control, show true when clicked
RAYGUIDEF bool GuiImageButton(GRectangle bounds, const char* text, Texture2D texture);                   // Image button control, returns true when clicked
RAYGUIDEF bool GuiImageButtonEx(GRectangle bounds, const char* text, Texture2D texture, GRectangle texSource);    // Image button extended control, returns true when clicked
RAYGUIDEF bool GuiToggle(GRectangle bounds, const char* text, bool active);                              // Toggle Button control, returns true when active
RAYGUIDEF int GuiToggleGroup(GRectangle bounds, const char* text, int active);                           // Toggle Group control, returns active toggle index
RAYGUIDEF bool GuiCheckBox(GRectangle bounds, const char* text, bool checked);                           // Check Box control, returns true when active
RAYGUIDEF int GuiComboBox(GRectangle bounds, const char* text, int active);                              // Combo Box control, returns selected item index
RAYGUIDEF bool GuiDropdownBox(GRectangle bounds, const char* text, int* active, bool editMode);          // Dropdown Box control, returns selected item
RAYGUIDEF bool GuiSpinner(GRectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode);     // Spinner control, returns selected value
RAYGUIDEF bool GuiValueBox(GRectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode);    // Value Box control, updates input text with numbers
RAYGUIDEF bool GuiTextBox(GRectangle bounds, char* text, int textSize, bool editMode);                   // Text Box control, updates input text
RAYGUIDEF bool GuiTextBoxMulti(GRectangle bounds, char* text, int textSize, bool editMode);              // Text Box control with multiple lines
RAYGUIDEF float GuiSlider(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue);       // Slider control, returns selected value
RAYGUIDEF float GuiSliderBar(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue);    // Slider Bar control, returns selected value
RAYGUIDEF float GuiProgressBar(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue);  // Progress Bar control, shows current progress value
RAYGUIDEF void GuiStatusBar(GRectangle bounds, const char* text);                                        // Status Bar control, shows info text
RAYGUIDEF void GuiDummyRec(GRectangle bounds, const char* text);                                         // Dummy control for placeholders
RAYGUIDEF int GuiScrollBar(GRectangle bounds, int value, int minValue, int maxValue);                    // Scroll Bar control
RAYGUIDEF Vector2 GuiGrid(GRectangle bounds, float spacing, int subdivs);                                // Grid control

// Advance controls set
RAYGUIDEF int GuiListView(GRectangle bounds, const char* text, int* scrollIndex, int active);            // List View control, returns selected list item index
RAYGUIDEF int GuiListViewEx(GRectangle bounds, const char** text, int count, int* focus, int* scrollIndex, int active);      // List View with extended parameters
RAYGUIDEF int GuiMessageBox(GRectangle bounds, const char* title, const char* message, const char* buttons);                 // Message Box control, displays a message
RAYGUIDEF int GuiTextInputBox(GRectangle bounds, const char* title, const char* message, const char* buttons, char* text);   // Text Input Box control, ask for text
RAYGUIDEF Color GuiColorPicker(GRectangle bounds, Color color);                                          // Color Picker control (multiple color controls)
RAYGUIDEF Color GuiColorPanel(GRectangle bounds, Color color);                                           // Color Panel control
RAYGUIDEF float GuiColorBarAlpha(GRectangle bounds, float alpha);                                        // Color Bar Alpha control
RAYGUIDEF float GuiColorBarHue(GRectangle bounds, float value);                                          // Color Bar Hue control
...
static bool CheckCollisionPointRec(Vector2 point, GRectangle rec);   // Check if point is inside rectangle
...
static void DrawRectangleRec(GRectangle rec, Color color);   // Draw rectangle filled with color
static void DrawRectangleLinesEx(GRectangle rec, int lineThick, Color color);    // Draw rectangle outlines
...
static GRectangle GetTextBounds(int control, GRectangle bounds)
{
    GRectangle textBounds = bounds;
...
static void GuiDrawText(const char* text, GRectangle bounds, int alignment, Color tint)
{
...
static void GuiDrawTooltip(GRectangle bounds)
{
    ...
    if (guiTooltipEnabled && (guiTooltip != NULL) && CheckCollisionPointRec(GetMousePosition(), bounds))
    {
        ...
        GRectangle tooltipBounds = { mousePosition.x, mousePosition.y, textSize.x + 20, textSize.y * 2 };
...
// Window Box control
bool GuiWindowBox(GRectangle bounds, const char* title)
{
...
#define WINDOW_STATUSBAR_HEIGHT        22
    ...
    GRectangle statusBar = { bounds.x, bounds.y, bounds.width, statusBarHeight };
    if (bounds.height < statusBarHeight * 2) bounds.height = statusBarHeight * 2;

    GRectangle closeButtonRec = { statusBar.x + statusBar.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) - 20,
    ...
    DrawRectangleRec(RAYGUI_CLITERAL(GRectangle) {
...
void GuiGroupBox(GRectangle bounds, const char* text)
{
...
#define GROUPBOX_TEXT_PADDING  10
    ...
    GuiLine(RAYGUI_CLITERAL(GRectangle) { bounds.x, bounds.y, bounds.width, 1 }, text);
    //--------------------------------------------------------------------
}

// Line control
void GuiLine(GRectangle bounds, const char* text)
{
...
// Panel control
void GuiPanel(GRectangle bounds)
{
...
GRectangle GuiScrollPanel(GRectangle bounds, GRectangle content, Vector2* scroll)
{
    GuiControlState state = guiState;
    ...
    const GRectangle horizontalScrollBar = { (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (float)bounds.x + verticalScrollBarWidth : (float)bounds.x) + GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)bounds.y + bounds.height - horizontalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)bounds.width - verticalScrollBarWidth - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)horizontalScrollBarWidth };
    const GRectangle verticalScrollBar = { (float)((GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ? (float)bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH) : (float)bounds.x + bounds.width - verticalScrollBarWidth - GuiGetStyle(DEFAULT, BORDER_WIDTH)), (float)bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), (float)verticalScrollBarWidth, (float)bounds.height - horizontalScrollBarWidth - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) };

    // Calculate view area (area without the scrollbars)
    GRectangle view = (GuiGetStyle(LISTVIEW, SCROLLBAR_SIDE) == SCROLLBAR_LEFT_SIDE) ?
        RAYGUI_CLITERAL(GRectangle) { bounds.x + verticalScrollBarWidth + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth } :
        RAYGUI_CLITERAL(GRectangle) { bounds.x + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.y + GuiGetStyle(DEFAULT, BORDER_WIDTH), bounds.width - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - verticalScrollBarWidth, bounds.height - 2 * GuiGetStyle(DEFAULT, BORDER_WIDTH) - horizontalScrollBarWidth };
...
// Label control
void GuiLabel(GRectangle bounds, const char* text)
{
    GuiControlState state = guiState;
...
bool GuiButton(GRectangle bounds, const char* text)
{
    GuiControlState state = guiState;
    bool pressed = false;
...
bool GuiLabelButton(GRectangle bounds, const char* text)
{
    GuiControlState state = guiState;
    bool pressed = false;
...
// Image button control, returns true when clicked
bool GuiImageButtonEx(GRectangle bounds, const char* text, Texture2D texture, GRectangle texSource)
{
    GuiControlState state = guiState;
    bool clicked = false;
...
bool GuiToggle(GRectangle bounds, const char* text, bool active)
{
    GuiControlState state = guiState;
...
int GuiToggleGroup(GRectangle bounds, const char* text, int active)
{
...
bool GuiCheckBox(GRectangle bounds, const char* text, bool checked)
{
    GuiControlState state = guiState;
...
int GuiComboBox(GRectangle bounds, const char* text, int active)
{
    GuiControlState state = guiState;

    bounds.width -= (GuiGetStyle(COMBOBOX, COMBO_BUTTON_WIDTH) + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING));

    GRectangle selector = { (float)bounds.x + bounds.width + GuiGetStyle(COMBOBOX, COMBO_BUTTON_PADDING),
...
bool GuiDropdownBox(GRectangle bounds, const char* text, int* active, bool editMode)
{
    GuiControlState state = guiState;
...
bool GuiTextBox(GRectangle bounds, char* text, int textSize, bool editMode)
{
...
bool GuiSpinner(GRectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode)
{
    GuiControlState state = guiState;
    ...
    GRectangle spinner = { bounds.x + GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_PADDING), bounds.y,
                          bounds.width - 2 * (GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH) + GuiGetStyle(SPINNER, SPIN_BUTTON_PADDING)), bounds.height };
    GRectangle leftButtonBound = { (float)bounds.x, (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
    GRectangle rightButtonBound = { (float)bounds.x + bounds.width - GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.y, (float)GuiGetStyle(SPINNER, SPIN_BUTTON_WIDTH), (float)bounds.height };
...
bool GuiValueBox(GRectangle bounds, const char* text, int* value, int minValue, int maxValue, bool editMode)
{
...
bool GuiTextBoxMulti(GRectangle bounds, char* text, int textSize, bool editMode)
{
    static int framesCounter = 0;           // Required for blinking cursor
    ...
    GRectangle cursor = { 0, 0, 1, GuiGetStyle(DEFAULT, TEXT_SIZE) + 2 };
...
float GuiSliderPro(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue, int sliderWidth)
{
    GuiControlState state = guiState;

    int sliderValue = (int)(((value - minValue) / (maxValue - minValue)) * (bounds.width - 2 * GuiGetStyle(SLIDER, BORDER_WIDTH)));

    GRectangle slider = { bounds.x, bounds.y + GuiGetStyle(SLIDER, BORDER_WIDTH) + GuiGetStyle(SLIDER, SLIDER_PADDING),
...
float GuiSlider(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, GuiGetStyle(SLIDER, SLIDER_WIDTH));
}

// Slider Bar control extended, returns selected value
float GuiSliderBar(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue)
{
    return GuiSliderPro(bounds, textLeft, textRight, value, minValue, maxValue, 0);
}

// Progress Bar control extended, shows current progress value
float GuiProgressBar(GRectangle bounds, const char* textLeft, const char* textRight, float value, float minValue, float maxValue)
{
    GuiControlState state = guiState;

    GRectangle progress = { bounds.x + GuiGetStyle(PROGRESSBAR, BORDER_WIDTH),
...
// Status Bar control
void GuiStatusBar(GRectangle bounds, const char* text)
{
    GuiControlState state = guiState;
    ...
    DrawRectangleRec(RAYGUI_CLITERAL(GRectangle) { bounds.x + GuiGetStyle(STATUSBAR, BORDER_WIDTH), bounds.y + GuiGetStyle(STATUSBAR, BORDER_WIDTH), bounds.width - GuiGetStyle(STATUSBAR, BORDER_WIDTH) * 2, bounds.height - GuiGetStyle(STATUSBAR, BORDER_WIDTH) * 2 }, Fade(GetColor(GuiGetStyle(STATUSBAR, (state != GUI_STATE_DISABLED) ? BASE_COLOR_NORMAL : BASE_COLOR_DISABLED)), guiAlpha));
...
void GuiDummyRec(GRectangle bounds, const char* text)
{
    GuiControlState state = guiState;
...
int GuiScrollBar(GRectangle bounds, int value, int minValue, int maxValue)
{
    GuiControlState state = guiState;
    ...
    // Arrow buttons [<] [>] [∧] [∨]
    GRectangle arrowUpLeft = { 0 };
    GRectangle arrowDownRight = { 0 };

    // Actual area of the scrollbar excluding the arrow buttons
    GRectangle scrollbar = { 0 };
    ...
    if ((state != GUI_STATE_DISABLED) && !guiLocked)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, bounds))
        {
            ...
            else if (IsMouseButtonDown(MOUSE_LEFT_BUTTON))
            {
                if (!isVertical)
                {
                    GRectangle scrollArea = { arrowUpLeft.x + arrowUpLeft.width, arrowUpLeft.y, scrollbar.width, bounds.height - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH) };
                ...
                {
                    GRectangle scrollArea = { arrowUpLeft.x, arrowUpLeft.y + arrowUpLeft.height, bounds.width - 2 * GuiGetStyle(SCROLLBAR, BORDER_WIDTH),  scrollbar.height };
...
// List View control
int GuiListView(GRectangle bounds, const char* text, int* scrollIndex, int active)
{
    int itemsCount = 0;
...
int GuiListViewEx(GRectangle bounds, const char** text, int count, int* focus, int* scrollIndex, int active)
{
    GuiControlState state = guiState;
...
// Color Panel control
Color GuiColorPanelEx(GRectangle bounds, Color color, float hue)
{
    GuiControlState state = guiState;
...
Color GuiColorPanel(GRectangle bounds, Color color)
{
...
float GuiColorBarAlpha(GRectangle bounds, float alpha)
{
#define COLORBARALPHA_CHECKED_SIZE   10

    GuiControlState state = guiState;
    GRectangle selector = { (float)bounds.x + alpha * bounds.width - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT), (float)bounds.height + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) * 2 };
...
float GuiColorBarHue(GRectangle bounds, float hue)
{
    GuiControlState state = guiState;
    GRectangle selector = { (float)bounds.x - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.y + hue / 360.0f * bounds.height - GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW), (float)bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_OVERFLOW) * 2, (float)GuiGetStyle(COLORPICKER, HUEBAR_SELECTOR_HEIGHT) };
...
//      Color GuiColorPanel(GRectangle bounds, Color color)
//      float GuiColorBarAlpha(GRectangle bounds, float alpha)
//      float GuiColorBarHue(GRectangle bounds, float value)
// NOTE: bounds define GuiColorPanel() size
Color GuiColorPicker(GRectangle bounds, Color color)
{
    color = GuiColorPanel(bounds, color);

    GRectangle boundsHue = { (float)bounds.x + bounds.width + GuiGetStyle(COLORPICKER, HUEBAR_PADDING), (float)bounds.y, (float)GuiGetStyle(COLORPICKER, HUEBAR_WIDTH), (float)bounds.height };
    //GRectangle boundsAlpha = { bounds.x, bounds.y + bounds.height + GuiGetStyle(COLORPICKER, BARS_PADDING), bounds.width, GuiGetStyle(COLORPICKER, BARS_THICK) };
...
// Message Box control
int GuiMessageBox(GRectangle bounds, const char* title, const char* message, const char* buttons)
{
...
int GuiTextInputBox(GRectangle bounds, const char* title, const char* message, const char* buttons, char* text)
{
...
Vector2 GuiGrid(GRectangle bounds, float spacing, int subdivs)
{
    ...
    if (tryBinary)
    {
            ...
            (signature[3] == ' '))
        {
            ...
            if (fontDataSize > 0)
            {
                ...
                fread(&whiteRec, 1, sizeof(GRectangle), rgsFile);
                ...
                // Load font recs data
                font.recs = (GRectangle*)RAYGUI_CALLOC(font.charsCount, sizeof(GRectangle));
                for (int i = 0; i < font.charsCount; i++) fread(&font.recs[i], 1, sizeof(GRectangle), rgsFile);
...
static bool CheckCollisionPointRec(Vector2 point, GRectangle rec)
{
...
static void DrawRectangleRec(GRectangle rec, Color color)
{
    DrawRectangle(rec.x, rec.y, rec.width, rec.height, color);
}

// Draw rectangle border lines with color
static void DrawRectangleLinesEx(GRectangle rec, int lineThick, Color color)
{
...
{
    GRectangle bounds = { (float)posX, (float)posY, (float)width, (float)height };
