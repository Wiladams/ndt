	...
	for (i = 1; i < gargc; i++)
	{
		...
		do
		{
			if (param[0] == '-' || param_continue)
			{
				switch (param[1])
				{
				case 'm': if (++i < gargc) ram_amt = SimpleReadNumberInt(gargv[i], ram_amt); break;
				case 'c': if (++i < gargc) instct = SimpleReadNumberInt(gargv[i], -1); break;
				case 'f': image_file_name = (++i < gargc) ? gargv[i] : 0; break;
				case 'b': dtb_file_name = (++i < gargc) ? gargv[i] : 0; break;
				...
				case 't': if (++i < gargc) time_divisor = SimpleReadNumberInt(gargv[i], 1); break;
				default:
					if (param_continue)
						param_continue = 0;
					else
