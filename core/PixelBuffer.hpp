...
private:
    ...
    const GRCOORD width;   //how many pixels wide
    const GRCOORD height;  // how many pixels tall
...
public:
    ...
    int getBitsPerPixel() const {return fBitsPerPixel;}
    
    virtual void * getData() = 0;
    virtual const void * getConstData() {return (const void *) getData();}
    virtual const int getDataLength() const = 0;
    virtual const void * getPixelPointer(GRCOORD x, GRCOORD y) const = 0;
    ...
    {
        for (GRCOORD col=x; col<x+width; col++) {
            setPixel(col,y, src);
        }
    }
    ...
    {
        for (GRSIZE offset =0; offset < width; offset++)
        {
    ...
    {
        for (GRCOORD row=0;row<height-1;row++)
        {
            for (GRCOORD col=0; col<width-1; col++)
            {
    ...
    virtual bool blend(const PixelBuffer &src, 
        GRCOORD srcX, GRCOORD srcY, int srcWidth, int srcHeight,
        GRCOORD destX, GRCOORD destY, int destWidth, int destHeight, const PixelTransferOp &tOp)
    {
