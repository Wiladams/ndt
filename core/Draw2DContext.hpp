...
/*
    A drawing context tied to blend2d
*/

class Draw2DContext {

private:
    PBDIBSection &fSurface;
    BLContext fContext;
    ...
    Draw2DContext() = delete;

public:
    Draw2DContext(PBDIBSection &surface)
    :fSurface(surface), 
    ...
    tOp(nullptr)
    {
        fContext = fSurface.getBlend2dContext();

        tOp = new PixelSRCOVER();
    }

    virtual ~Draw2DContext()
    {
    }
    ...
    void setBlendMode(uint32_t compOp)
    {
    ...
    bool clear()
    {
        fSurface.getBlend2dContext().clearAll();
