...
public:
        ...
        , fDataLength(0)
    {
        ...
        fDataLength = sizeof(PixRGBA) * (width * height);
    ...
    virtual const void * getPixelPointer(GRCOORD x, GRCOORD y) const
    {
        ...
        int destX, int destY, int destWidth, int destHeight)
    {
        ...
        if (isOptimal) {
            ...
            for (GRCOORD row=clipRect.y1; row<clipRect.y2; row++) {
                ...
                // setSpan on ourself
                setSpan((GRCOORD)clipRect.x1, row, (GRSIZE)clipRect.getWidth(), pixPtr);
            }
        } else {
